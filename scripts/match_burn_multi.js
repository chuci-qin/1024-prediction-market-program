/**
 * MatchBurnMulti - Match N sell orders to burn complete set for multi-outcome market
 * 
 * Usage: node match_burn_multi.js [market_id] [amount] [order1_id:price] [order2_id:price] ...
 * 
 * Example (3-outcome market):
 *   node match_burn_multi.js 5 10000000 201:0.35 202:0.35 203:0.30
 *   
 * Requires: sum of all prices ≈ 1.0 (within tolerance) for proper settlement
 */
...
// MatchBurnMulti = instruction index 43
const MATCH_BURN_MULTI_IX = 43;

const PRICE_PRECISION = 1_000_000;

/**
 * Serialize MatchBurnMultiArgs (Borsh format)
 * Layout:
 * - u8 instruction (43)
 * - u64 market_id
 * - u8 num_outcomes
 ...
 */
function serializeMatchBurnMultiArgs(marketId, numOutcomes, amount, orders) {
  const orderDataSize = orders.length * (1 + 8 + 8);
  const buffer = Buffer.alloc(1 + 8 + 1 + 8 + 4 + orderDataSize);
  let offset = 0;
  
  buffer.writeUInt8(MATCH_BURN_MULTI_IX, offset); offset += 1;
...
async function main() {
  ...
  console.log('1024 Prediction Market - MatchBurnMulti (Multi-Outcome)');
  console.log('='.repeat(60));
  
  const marketId = process.argv[2] ? parseInt(process.argv[2]) : 1;
  const amount = process.argv[3] ? parseInt(process.argv[3]) : 10_000_000;
  ...
  if (orders.length === 0) {
    ...
    orders.push({ outcomeIndex: 0, orderId: 101, price: 700_000 }); // YES Sell @ $0.70
    orders.push({ outcomeIndex: 1, orderId: 102, price: 300_000 }); // NO Sell @ $0.30
  }
  ...
  console.log(`\nMarket ID: ${marketId}`);
  console.log(`Amount: ${amount / 1_000_000} tokens to burn`);
  console.log(`Number of Outcomes: ${numOutcomes}`);
  console.log(`Sell Orders:`);
  orders.forEach((o, i) => {
    console.log(`  ${i}: Order #${o.orderId} @ $${(o.price / PRICE_PRECISION).toFixed(4)}`);
  });
  console.log(`Total Price (USDC to release): $${(totalPrice / PRICE_PRECISION).toFixed(6)}`);
  
  if (Math.abs(totalPrice - PRICE_PRECISION) > 10000) {
  ...
  // For MatchBurnMulti, each outcome needs: Order, Mint, Seller's Escrow Token Account
  for (const order of orders) {
    ...
    // Seller's Escrow Token Account PDA (holds tokens to be burned)
    ...
    keys.push({ pubkey: escrowPda, isSigner: false, isWritable: true });
  }
  
  const instructionData = serializeMatchBurnMultiArgs(marketId, numOutcomes, amount, orders);
  console.log(`\nInstruction data (${instructionData.length} bytes): ${instructionData.toString('hex').slice(0, 60)}...`);
  
  const matchBurnMultiIx = new TransactionInstruction({
    programId: PROGRAM_ID,
    keys: keys,
    data: instructionData,
  });
  
  const tx = new Transaction().add(matchBurnMultiIx);
  
  console.log('\nSending MatchBurnMulti transaction...');
  
  try {
    const { signature, confirmed } = await sendTransactionFast(connection, tx, [caller]);
    
    if (confirmed) {
      console.log('\n✅ MatchBurnMulti successful!');
    } else {
